        ...
        <dialog id="quota_modal" class="modal" open>
            <div class="modal-box">
                ...
                @if(($quota['tier'] ?? 'normal') === 'premium')
                    <p class="py-3 text-sm">{{ __('file.quota_premium_body') }}</p>
                    <a href="mailto:{{ config('storage.support_email') }}" class="link link-primary text-sm">
                        {{ config('storage.support_email') }}
                    </a>
                @else
                    <p class="py-3 text-sm">{{ __('file.quota_normal_body', ['days' => config('storage.trash_retention_days')]) }}</p>
                    <ul class="list-disc list-inside text-sm text-base-content/70 space-y-1">
                        <li>{{ __('file.quota_tip_delete') }}</li>
                        <li>{{ __('file.quota_tip_wait', ['days' => config('storage.trash_retention_days')]) }}</li>
                        <li>{{ __('file.quota_tip_upgrade') }}</li>
                    </ul>
                @endif

                <div class="modal-action">
                    <form method="dialog">
                        <button class="btn">{{ __('file.quota_ok') }}</button>
                    </form>
                </div>
            </div>
        </dialog>
