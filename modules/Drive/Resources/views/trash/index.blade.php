...
<div class="max-w-5xl mx-auto p-6">
    ...
    <h1 class="text-2xl font-bold mb-6">{{ __('drive::trash.title') }}</h1>
    ...
    @if($folders->isEmpty() && $files->isEmpty())
        <div class="flex flex-col items-center justify-center h-64 text-base-content/40">
            ...
            <p class="text-lg">{{ __('drive::trash.empty') }}</p>
        </div>
    @endif
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                <form method="POST"
                                      ...
                                      class="inline">
                                    @csrf @method('PATCH')
                                    <button class="btn btn-ghost btn-xs">{{ __('drive::trash.restore') }}</button>
                                </form>
                                @php($folderAvailableAt = $folder->deleted_at->copy()->addDays($retentionDays))
                                @if(!$bypassLock && $folderAvailableAt->isFuture())
                                    <span class="tooltip tooltip-left" data-tip="{{ __('drive::trash.retention_locked', ['time' => $folderAvailableAt->diffForHumans()]) }}">
                                        <button class="btn btn-ghost btn-xs text-error" disabled>
                                            {{ __('drive::trash.delete_permanently') }}
                                        </button>
                                    </span>
                                @else
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                ...
                                @if(!$bypassLock && $fileAvailableAt->isFuture())
                                    <span class="tooltip tooltip-left" data-tip="{{ __('drive::trash.retention_locked', ['time' => $fileAvailableAt->diffForHumans()]) }}">
                                        <button class="btn btn-ghost btn-xs text-error" disabled>
                                            {{ __('drive::trash.delete_permanently') }}
                                        </button>
                                    </span>
...
<dialog id="modal_permanent_delete" class="modal">
    <div class="modal-box">
        <h3 class="font-bold text-lg mb-2 text-error">{{ __('drive::trash.delete_permanently') }}</h3>
        <p class="text-base-content/70 mb-4">
            {{ __('drive::trash.confirm_permanent') }} <strong id="permanent_delete_name"></strong>?
        </p>
