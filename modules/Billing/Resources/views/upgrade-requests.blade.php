...
<div class="flex h-[calc(100vh-64px)]">
    ...
    <main class="flex-1 overflow-y-auto p-6">
        ...
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold">{{ __('billing::upgrade.admin_title') }}</h1>
        ...
        <div class="overflow-x-auto rounded-xl border border-base-200">
            <table class="table table-zebra w-full">
                <thead>
                    <tr>
                        <th>{{ __('billing::upgrade.user') }}</th>
                        <th>{{ __('billing::upgrade.reason') }}</th>
                        <th class="text-end">{{ __('billing::upgrade.actions') }}</th>
                    </tr>
                </thead>
                <tbody>
                            ...
                            <td class="text-end">
                                <div class="flex gap-2 justify-end">
                                    <form method="POST" action="{{ route('admin.upgrade-requests.approve', $req) }}">
                                        @csrf @method('PATCH')
                                        <button class="btn btn-success btn-xs">{{ __('billing::upgrade.approve') }}</button>
                                    </form>
                                    <form method="POST" action="{{ route('admin.upgrade-requests.reject', $req) }}">
                                        @csrf @method('PATCH')
                                        <button class="btn btn-error btn-xs btn-outline">{{ __('billing::upgrade.reject') }}</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="text-center py-12 text-base-content/40">{{ __('billing::upgrade.empty') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
