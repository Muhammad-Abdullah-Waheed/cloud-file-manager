...
<div class="flex h-[calc(100vh-64px)]">
        ...
        @if(auth()->user()->hasPermission('manage-users'))
            <a href="{{ route('admin.upgrade-requests.index') }}"
               class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-base-300 text-sm">
                ...
                {{ __('admin::admin.upgrade_requests') }}
            </a>
        @endif
    ...
    <main class="flex-1 overflow-y-auto p-6">
        ...
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold">{{ __('admin::admin.delete_requests') }}</h1>
        ...
        <div class="overflow-x-auto rounded-xl border border-base-200">
            <table class="table table-zebra w-full">
                <thead>
                    <tr>
                        @if($viewMode === 'admin')
                            <th>{{ __('admin::admin.requested_by') }}</th>
                        @endif
                        <th>{{ __('admin::admin.target') }}</th>
                        <th>{{ __('admin::admin.justification') }}</th>
                        <th>{{ __('admin::admin.status') }}</th>
                        <th>{{ __('admin::admin.date') }}</th>
                        @if($viewMode === 'admin')
                            <th>{{ __('admin::admin.actions') }}</th>
                        @else
                            <th>{{ __('admin::admin.actioned_by') }}</th>
                        @endif
                    </tr>
                </thead>
                <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($req->status === 'pending')
                                    <span class="badge badge-warning badge-sm">{{ __('admin::admin.pending') }}</span>
                                @elseif($req->status === 'approved')
                                    <span class="badge badge-success badge-sm">{{ __('admin::admin.approved') }}</span>
                                @else
                                    <span class="badge badge-error badge-sm">{{ __('admin::admin.rejected') }}</span>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center py-12 text-base-content/40">
                                {{ __('admin::admin.no_delete_requests') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
