...
<div class="flex h-[calc(100vh-64px)]">
    ...
    <aside class="w-64 bg-base-200 border-e border-base-300 flex-col p-4 gap-2 hidden md:flex">
        ...
        <a href="{{ route('admin.delete-requests.index') }}"
           class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-base-300 text-sm">
            ...
            {{ __('admin::admin.delete_requests') }}
        </a>
            ...
            <div class="card-body p-4 gap-1">
                ...
                @php
                    $pct     = $user->storage_limit > 0
                        ...
                        : 0;
                    $usedKB  = number_format($user->storage_used / 1024, 2);
                    $limitKB = number_format($user->storage_limit / 1024, 2);
                @endphp
                <div class="mt-3">
                    <div class="text-xs text-base-content/50 mb-1">{{ __('drive::dashboard.storage') }}</div>
                    <progress class="progress progress-primary w-full" value="{{ $pct }}" max="100"></progress>
                    <div class="text-xs text-base-content/50 mt-1">{{ $usedKB }} / {{ $limitKB }} KB</div>
                </div>
            </div>
        </div>
    ...
    <main class="flex-1 overflow-y-auto p-6">
        ...
        <div class="text-sm breadcrumbs mb-4">
            <ul>
                <li>
                    <a href="{{ route('admin.users.index') }}">{{ __('admin::admin.users') }}</a>
                </li>
                <li>
                        ...
                        @elseif(auth()->user()->hasRole('manager'))
                            <div class="absolute top-1 end-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onclick="openRequestDeleteModal('folder', {{ $folder->id }}, '{{ addslashes($folder->name) }}')"
                                        ...
                                        title="{{ __('admin::admin.request_delete') }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4"
        ...
        @if($folders->isEmpty() && $files->isEmpty())
            <div class="flex flex-col items-center justify-center h-96 text-base-content/40">
                ...
                <p class="text-sm mt-1">{{ __('admin::admin.user_has_no_files', ['name' => $user->name]) }}</p>
            </div>
        @endif
...
<dialog id="modal_request_delete" class="modal">
    <div class="modal-box">
        <h3 class="font-bold text-lg mb-1">{{ __('admin::admin.request_delete') }}</h3>
        <p class="text-base-content/60 text-sm mb-4">
            {{ __('admin::admin.request_delete_for') }}:
        ...
        <form method="POST" action="{{ route('admin.delete-requests.store') }}" id="request_delete_form">
            ...
            <div class="form-control mb-4">
                <label class="label">
                    <span class="label-text font-medium">{{ __('admin::admin.justification') }}</span>
                </label>
                <textarea name="reason" rows="4" required minlength="10"
                          placeholder="{{ __('admin::admin.justification_placeholder') }}"
