...
<div class="max-w-4xl mx-auto p-6">
    ...
    <h1 class="text-2xl font-bold mb-6">{{ __('sharing::share.shared_with_me') }}</h1>
    ...
    @if($sharedWithMe->isEmpty())
        <div class="flex flex-col items-center justify-center h-64 text-base-content/40">
            <p class="text-lg">{{ __('sharing::share.no_shares') }}</p>
        </div>
    @else
        <div class="overflow-x-auto rounded-xl border border-base-200">
            <table class="table w-full">
                <thead>
                    <tr>
                        <th>{{ __('file.name') }}</th>
                        <th>{{ __('sharing::share.shared_by') }}</th>
                        <th>{{ __('sharing::share.permission') }}</th>
                    </tr>
                </thead>
                <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge {{ $share->permission === 'write' ? 'badge-primary' : 'badge-ghost' }}">
                                    {{ __('sharing::share.' . $share->permission) }}
                                </span>
                            </td>
                        </tr>
                    @endforeach
